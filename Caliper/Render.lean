...
instruction per line, memory operations under the `mem.` qualifier (the register
file, a future phase, will use `reg.`), destination first, buffers as `b<i>`,
registers as `r<i>`, two-space indentation for block bodies.
